...
if ($_SERVER['PHP_AUTH_USER'] == $GLOBALS['tokenname'] && $_SERVER['PHP_AUTH_PW'] == $GLOBALS['tokenvalue']) {
    ...
    if ((!empty($_POST["blog_id"])) && (isset($_POST["upvote"])) && (!empty($_POST["user_id"]))) {
        ...
        $query_blog_action = mysqli_query($conn, "select * from tbl_blog_action where blog_id='{$blogid}' and user_id='{$user_id}' ") or die(mysqli_error($conn));


        $count = mysqli_num_rows($query_blog_action);
        if ($count > 0) {
            $row_action = mysqli_fetch_array($query_blog_action);
            $is_upvote_downvote = $row_action["is_upvote_downvote"];

            
            $query_blog = mysqli_query($conn,"SELECT * FROM `tbl_blog` where blog_id='{$blogid}'")or die(mysqli_error($conn));
              $row_blog = mysqli_fetch_array($query_blog);
            ...
            if ($upvote == 0) {
                       ...
                       $downvote= ($row_blog["downvote"]+1);
                 $upvote_data= ($row_blog["upvote"]-1);
                   
              
                  $query_upvote_update = mysqli_query($conn, "UPDATE `tbl_blog_action` SET `is_upvote_downvote`='0',`update_datetime`='{$today_datetime}' WHERE `blog_action_id`='{$row_action["blog_action_id"]}'") or die(mysqli_error($conn));
                   
                   $querycontactupdate = mysqli_query($conn, "UPDATE `tbl_blog` SET `upvote`='$upvote_data',downvote='{$downvote}',is_upvote='0' WHERE blog_id='{$blogid}'") or die(mysqli_error($conn));
            ...
            //update upvote
                 ...
                 $upvote_data= ($row_blog["upvote"]+1);
                  ...
                  $query_upvote_update = mysqli_query($conn, "UPDATE `tbl_blog_action` SET `is_upvote_downvote`='1',`update_datetime`='{$today_datetime}' WHERE `blog_action_id`='{$row_action["blog_action_id"]}'") or die(mysqli_error($conn));
                   
                   $querycontactupdate = mysqli_query($conn, "UPDATE `tbl_blog` SET `upvote`='$upvote_data',downvote='{$downvote}',is_upvote='1' WHERE blog_id='{$blogid}'") or die(mysqli_error($conn));
