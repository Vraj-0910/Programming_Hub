...
//        }
          ...
          $query_user_type_1 = mysqli_query($conn, "select * from tbl_questions where  is_delete='0' and user_id != '$userid' and is_active='1' $search order by question_id DESC") or die(mysqli_error($conn));
...
//        }
         ...
         $query_user_type = mysqli_query($conn, "select * from tbl_questions where user_id != '$userid' and is_delete='0' and is_active='1' $search order by question_id DESC limit $offset, $rowsperpage") or die(mysqli_error($conn));
     
    //     echo "select * from tbl_questions where user_id != '$userid' and is_delete='0' and is_active='1' $search order by question_id DESC limit $offset, $rowsperpage";
      ...
      //this condition check current page less then qual total page or totalpage qual to 0
         ...
         //$query_remove = mysqli_query($conn,"select * from tbl_remove_user_question user_id='{$userid}'");
...
//		$response["message"] = "You're all caught up. No questions are available for you at the moment";
        ...
        if($myarray1 =='')
    {
       ...
       //$response["question"] ="No";     
    }
       else{ 
            $response["question"] =$myarray1;
       }
    }
    //no record found
    else {
